...
# ),
# cte2 as(
#     select *, date_add(cte.first_date, interval 1 day) as next_day
#     from cte
# )
# SELECT ROUND(
#     (SELECT COUNT(DISTINCT player_id) 
#      FROM Activity 
#      WHERE (player_id, next_date) IN (SELECT player_id, next_day FROM cte2))
#     / COUNT(DISTINCT player_id) FROM Activity, 2)
# AS fraction;
